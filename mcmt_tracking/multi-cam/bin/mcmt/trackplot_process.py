...
class SingleCameraTrackPlot(Process):
    ...
    def __init__(self, filename, index, queue, FPS, FRAME_WIDTH, FRAME_HEIGHT, SCALE_FACTOR):
        ...
        self.recording = cv2.VideoWriter('../data/video_plot.mp4', cv2.VideoWriter_fourcc(*'MJPG'),
...
class MultiCameraTrackPlot(Process):
    ...
    def run(self):
        for index, filename in enumerate(self.filenames):
            if index == 0:
                self.cap1 = cv2.VideoCapture(filename)
                self.cap1.set(3, self.frame_w)
                self.cap1.set(4, self.frame_h)
                self.cap1.set(5, self.fps)
                self.cap1.release()

            else:
                self.cap2 = cv2.VideoCapture(filename)
                self.cap2.set(3, self.frame_w)
                self.cap2.set(4, self.frame_h)
                self.cap2.set(5, self.fps)
                self.cap2.release()
                self.last_update = time.time()
